...
-(IBAction) exceptionHit:(id) sender {
    @try {
        [NSException raise:@"test handled exception" format:@"HE test"];
    }
    @catch (NSException *exception) {
        // Log a handled exception to Crittercism
        [Crittercism logHandledException:exception];
...
-(IBAction) startTransaction:(id) sender {
    [Crittercism beginTransaction:@"Checkpoint1"];
}

-(IBAction) endTransaction:(id) sender {
    [Crittercism endTransaction:@"Checkpoint1"];
}

-(IBAction) failTransaction:(id) sender {
    [Crittercism failTransaction:@"Checkpoint1"];
}

-(IBAction) heFailTxN:(id) sender {
    @try {
        [NSException raise:@"test handled exception" format:@"Checkpoint1"];
